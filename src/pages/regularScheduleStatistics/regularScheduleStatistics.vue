...
<script>
import { regularStatisticsHeadSearch, regularStatisticsTableHead } from '@/utils/data'
...
export default {
  name: 'regularScheduleStatistics',
  props: {
    searchData: {
      type: Array,
      required: true
  ...
  data () {
    return {
      formModules: regularStatisticsHeadSearch,
      tableList: [],
      page: {
        pageNum: 1,
        pageSize: 10,
  ...
  mounted () {
    ...
    regularStatisticsHeadSearch[0].data = [thisDay.dayStr, thisDay.dayStr]
    ...
    this.$emit('getInfo', regularStatisticsHeadSearch)
    this.$emit('getTable', regularStatisticsTableHead)
  ...
  methods: {
    ...
    getTableData (params) {
      ...
      net.apiGet(api.orderManage.regularScheduleStatisticsList, params).then(data => {
    ...
    exportOrder () {
      ...
      str = api.orderManage.regularScheduleStatisticsOut + '?' + str
