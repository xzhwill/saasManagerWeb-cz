  ...
  <div class="common-container">
    <div class="page-nav-btn clearfix">
      <div class="page-nav-btn-item fl" v-if="rmList.indexOf(operateInfo.create.operationId)> -1">
        <el-button @click="$router.push('./addSchedule')" type="primary">新建班次</el-button>
          ...
          <el-table-column align="center" label="操作" width="180px">
            <template slot-scope="scope">
              <el-row class="action-btn-group">
                <span @click="modify(scope.row)" class="action-btn blue">修改</span>
                <span
...
import { scheduleManageHeadSearch, scheduleManageTableHead } from '@/utils/data'
...
export default {
  ...
  data () {
    return {
      formModules: scheduleManageHeadSearch,
      tableList: [],
      checkedRow: [],
      page: {
        pageNum: 1,
  ...
  mounted () {
    ...
    //       }
    //       scheduleManageHeadSearch.forEach(val => {
    //         if (val.name == key) {
    //           if (key === 'classType') {
    ...
    // } else {
    scheduleManageHeadSearch.forEach(item => {
    ...
    // }
    this.$emit('getInfo', scheduleManageHeadSearch, 'line')
    this.$emit('getTable', scheduleManageTableHead)
