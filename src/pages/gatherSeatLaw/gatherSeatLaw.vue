...
export default {
  ...
  data () {
    ...
    return {
      // formModules: gatherBusHeadSearch,
      tableList: [],
      page: {
        pageNum: 1,
        pageSize: 10,
  ...
  mounted () {
    ...
    this.rmList = setOperation('gatherSeatLaw')
    // this.$emit('getInfo', gatherBusHeadSearch)
    // this.$emit('getTable', gatherBusTableHead)
  ...
  methods: {
    getCompanyList () {
      net.apiGet(api.channel.option).then(data => {
        if (data && data.code === enums.NET_SUCCESS) {
          ...
          // gatherBusTableHead[(gatherBusHeadSearch.length - orderReportForms)].option = this.options
        } else {
          help.E(data.msg)
        }
      })
    ...
    initCheckList () {
      // gatherBusTableHead.forEach(item => {
      //   // 默认全选
    ...
    getTableData (params) {
      ...
      net.apiGet(api.gatherBus.list, params).then(data => {
        if (data && data.code === enums.NET_SUCCESS) {
