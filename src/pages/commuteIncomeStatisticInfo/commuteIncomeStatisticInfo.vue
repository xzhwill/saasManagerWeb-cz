...
<script>
import { commuteInfoHeadSearch, commuteInfoTableHead } from '@/utils/data'
import { setOperation, getYearMonthTime } from '@/utils/public'
...
export default {
  name: 'commuteIncomeStatisticInfo',
  props: {
    searchData: {
      type: Array,
      required: true
  ...
  data () {
    return {
      formModules: commuteInfoHeadSearch,
      tableList: [],
      page: {
        pageNum: 1,
        pageSize: 10,
  ...
  mounted () {
    ...
    let thisDay = getYearMonthTime()
    commuteInfoHeadSearch[0].data = [thisDay.dayStr + '-01', thisDay.dayStr + '-' + thisDay.days]
    // this.searchData.startTime = thisDay.dayStr + '-01 00:00:00'
    // this.searchData.endTime = thisDay.dayStr + '-' + thisDay.days + ' 23:59:59'
    // this.getTableData(this.searchData)
    this.$emit('getInfo', commuteInfoHeadSearch)
    this.$emit('getTable', commuteInfoTableHead)
  ...
  methods: {
    ...
    getTableData (params) {
      ...
      net.apiGet(api.orderManage.commuteIncomeStatisticList, params).then(data => {
    ...
    exportOrder () {
      ...
      str = api.orderManage.commuteIncomeStatisticOut + '?' + str
