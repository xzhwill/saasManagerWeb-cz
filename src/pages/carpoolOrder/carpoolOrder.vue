        ...
        <el-table header-row-class-name="tableHeader" class="page-table-main" :data="tableList" stripe>
          <el-table-column v-for="(item, index) in tableHeader" :key="index" :label="item.label" align="center"
            ...
            <template slot-scope="scope">
              ...
              <span
                v-if="item.name !='state' && item.name!='verification' && item.name !='checkTicketState'">{{ scope.row[item.name] }}</span>
            </template>
          </el-table-column>
        </el-table>
        <div class="page-page">
...
export default {
  ...
  methods: {
    ...
    getTableData (params) {
      ...
      net.apiGet(api.carpoolOrder.tableList, params).then(data => {
    ...
    exportOrder () {
      ...
      str = api.carpoolOrder.export + '?' + str
