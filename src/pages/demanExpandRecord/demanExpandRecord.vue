        ...
        <el-table header-row-class-name="tableHeader" class="page-table-main" :data="distributionList" stripe :row-class-name="tableRowClassName">
          <el-table-column v-for="(item, index) in tableHeader" :key="index" :label="item.label" :prop="item.name" v-if="tableHeaderCheckList.indexOf(item.label) !== -1"
            ...
            <template slot-scope="scope">
              <span v-if="item.name === 'tradeChannel'">{{scope.row[item.name] == 1 ? '支付宝' : (scope.row[item.name] == 2 ? '微信' : '企业支付')}}</span>
...
import { demanExpandRecordHeadSearch, demanExpandRecordTableHead } from '@/utils/data'
...
export default {
  ...
  mounted () {
    ...
    //       }
    //       demanExpandRecordHeadSearch.forEach(val => {
    //         if (val.name == key) {
    //           if (key === 'state') {
    ...
    // } else {
    demanExpandRecordHeadSearch.forEach(item => {
    ...
    // }
    this.$emit('getInfo', demanExpandRecordHeadSearch)
    this.$emit('getTable', demanExpandRecordTableHead)
  ...
  methods: {
    ...
    getTableData (params) {
      ...
      request.apiGet(api.demanExpandRecord.list, params).then(data => {
        if (data.code === enums.NET_SUCCESS) {
          sessionStorage.removeItem('demanExpandRecordFlag')
