...
import { orderReportHeadSearch, orderReportListTableHead } from '@/utils/data'
...
export default {
  ...
  data () {
    return {
      formModules: orderReportHeadSearch,
      tableList: [],
      checkedRow: [],
      page: {
        pageNum: 1,
  ...
  mounted () {
    ...
    this.$emit('getInfo', orderReportHeadSearch)
    this.$emit('getTable', orderReportListTableHead)
  ...
  methods: {
    doClearObj () {
      ...
      orderReportHeadSearch.forEach(item => {
    ...
    getTableData (params) {
      ...
      net.apiGet(api.report.orderList, params).then(data => {
