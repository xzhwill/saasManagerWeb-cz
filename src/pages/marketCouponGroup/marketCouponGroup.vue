  ...
  <div class="common-container">
    <div class="page-nav-btn clearfix">
      <div class="page-nav-btn-item fl">
        <el-button type="primary" @click="$router.push('./addMarketCouponGroup')" v-if="rmList.indexOf(operateInfo.create.operationId) > -1">新建组合模板</el-button>
        <el-button
          @click="groupDel"
          type="danger"
...
import { marketCouponGroupHeadSearch, marketCouponGroupTableHead } from '@/utils/data'
...
export default {
  ...
  mounted () {
    // let searchData = sessionStorage.getItem('couponGroup')
    // if (sessionStorage.getItem('couponGroupFlag') && searchData) {
    ...
    //       marketCouponGroupHeadSearch.forEach(val => {
    //         if (val.name == key) {
    //           val.data = item[key]
    //         }
    //       })
    ...
    // }
    this.rmList = setOperation('marketCouponGroup')
    this.$emit('getInfo', marketCouponGroupHeadSearch)
    this.$emit('getTable', marketCouponGroupTableHead)
  ...
  methods: {
    ...
    forbidden (item) {
      ...
      net.apiPost(api.couponGroup.forbidden, params).then(data => {
    ...
    groupDel () {
      this.$confirm('此操作将永久删除且不可恢复, 是否继续?', '提示', {
        ...
        net.apiPost(api.couponGroup.del, { templateIdList: this.checkedRow }).then(data => {
          if (data.code === enums.NET_SUCCESS) {
            help.S('删除成功')
            this.page.pageNum = 1
            this.getTableData(this.page)
    ...
    modify (templateId) {
      ...
      sessionStorage.setItem('couponGroup', JSON.stringify(searchArr))
    ...
    getTableData (params) {
      ...
      net.apiGet(api.couponGroup.list, params).then(data => {
        if (data.code === enums.NET_SUCCESS) {
          sessionStorage.removeItem('couponGroupFlag')
