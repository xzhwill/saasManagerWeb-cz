...
<script>
import { charterInfoHeadSearch, charterInfoTableHead } from '@/utils/data'
...
export default {
  name: 'charterIncomeStatisticInfo',
  props: {
    searchData: {
      type: Array,
      required: true
  ...
  data () {
    return {
      formModules: charterInfoHeadSearch,
      tableList: [],
      page: {
        pageNum: 1,
        pageSize: 10,
  ...
  mounted () {
    ...
    charterInfoHeadSearch[0].data = [thisDay.dayStr + '-01', thisDay.dayStr + '-' + thisDay.days]
    // this.searchData.startTime = thisDay.dayStr + '-01 00:00:00'
    // this.searchData.endTime = thisDay.dayStr + '-' + thisDay.days + ' 23:59:59'
    // this.getTableData(this.searchData)
    this.$emit('getInfo', charterInfoHeadSearch)
    this.$emit('getTable', charterInfoTableHead)
  ...
  methods: {
    ...
    getTableData (params) {
      ...
      net.apiGet(api.orderManage.charterIncomeStatisticList, params).then(data => {
    ...
    exportOrder () {
      ...
      str = api.orderManage.charterIncomeStatisticOut + '?' + str
