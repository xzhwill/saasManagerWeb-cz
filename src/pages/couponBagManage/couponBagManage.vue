  ...
  <div class="common-container">
    ...
    <div class="page-nav-btn clearfix">
      <div class="page-nav-btn-item fl">
        <el-button type="primary" @click="$router.push('./addCouponBag')" v-if="rmList.indexOf(operateInfo.create.operationId) > -1">新建礼包</el-button>
        ...
        <el-table stripe header-row-class-name="tableHeader" class="page-table-main" :data="tableList" @selection-change="handleSelectionChange">@selection-change="handleSelectionChange">
          ...
          <el-table-column show-overflow-tooltip v-for="(item, index) in tableHeader" :key="index" :label="item.label"
            ...
            <template slot-scope="scope">
              ...
              <span v-else-if="item.name==='activityLink'" class="clipboard-text" :data-clipboard-text="scope.row[item.name]" @click="clipBoardContentClick()">{{ scope.row[item.name]?scope.row[item.name]:'-' }}</span>
...
import { couponBagHeadSearch, couponBagTableHead } from '@/utils/data'
...
export default {
  ...
  data () {
    return {
      formModules: couponBagHeadSearch,
      tableList: [],
      checkedRow: [],
      page: {
        pageNum: 1,
  ...
  mounted () {
    couponBagHeadSearch.forEach(item => {
    ...
    this.$emit('getInfo', couponBagHeadSearch);
    this.$emit('getTable', couponBagTableHead);
    this.$emit('getPage', this.page);
    this.rmList = setOperation('couponBagManage');
  },
  methods: {
    ...
    handleSelectionChange (checkedArr) { // 批量删除数组变化
      ...
      checkedArr.forEach((item, index) => {
        checkGroupDriverIds.push(item.couponBagId);
      })
    ...
    modify (row) { // 去详情页面进行修改
      ...
      sessionStorage.setItem('couponBagSearch', JSON.stringify(searchArr));
      this.$router.push({
        name: 'addCouponBag',
        query: { 
    ...
    doSearch () {
      ...
      this.page.pageNum = 1;
      sessionStorage.removeItem('couponBagSearch');
      this.getTableData(this.searchData);
    },
    doClearObj () {
    ...
    getTableData (params) {
      ...
      // if(paramsNot.effectiveBeginDate===""){
      //   delete paramsNot.effectiveBeginDate
      // }
      // if(paramsNot.effectiveBeginDate===""){
      //   delete paramsNot.effectiveBeginDate
      // }
      ...
      net.apiPost(api.giftPacksActivity.query, isArray?paramsNot:params).then(data => {
        if (data.code === enums.NET_SUCCESS) {
          this.tableList = data.data.list;
          for(let i=0;i<this.tableList.length;i++){
            this.tableList[i].createTime=showDateStyle(this.tableList[i].effectiveDate)+" - "+showDateStyle(this.tableList[i].expiryDate)
          }
