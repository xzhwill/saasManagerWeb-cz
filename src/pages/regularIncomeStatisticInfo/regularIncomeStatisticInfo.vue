...
<script>
import { regularInfoHeadSearch, regularInfoTableHead } from '@/utils/data'
...
export default {
  name: 'regularIncomeStatisticInfo',
  props: {
    searchData: {
      type: Array,
      required: true
  ...
  data () {
    return {
      formModules: regularInfoHeadSearch,
      tableList: [],
      page: {
        pageNum: 1,
        pageSize: 10,
  ...
  mounted () {
    ...
    regularInfoHeadSearch[0].data = [thisDay.dayStr + '-01', thisDay.dayStr + '-' + thisDay.days]
    // this.searchData.startTime = thisDay.dayStr + '-01 00:00:00'
    // this.searchData.endTime = thisDay.dayStr + '-' + thisDay.days + ' 23:59:59'
    // this.getTableData(this.searchData)
    this.$emit('getInfo', regularInfoHeadSearch)
    this.$emit('getTable', regularInfoTableHead)
  ...
  methods: {
    ...
    getTableData (params) {
      ...
      net.apiGet(api.orderManage.regularIncomeStatisticList, params).then(data => {
    ...
    exportOrder () {
      ...
      str = api.orderManage.regularIncomeStatisticOut + '?' + str
