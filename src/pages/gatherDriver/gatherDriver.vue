...
export default {
  ...
  data () {
    ...
    return {
      // formModules: gatherDriverHeadSearch,
      tableList: [],
      page: {
        pageNum: 1,
        pageSize: 10,
  ...
  mounted () {
    ...
    this.rmList = setOperation('gatherDriver')
    // this.$emit('getInfo', gatherDriverHeadSearch)
    // this.$emit('getTable', gatherDriverTableHead)
  ...
  methods: {
    getCompanyList () {
      net.apiGet(api.channel.option).then(data => {
        if (data && data.code === enums.NET_SUCCESS) {
          ...
          // gatherDriverHeadSearch[(gatherDriverHeadSearch.length - orderReportForms)].option = this.options
        } else {
          help.E(data.msg)
        }
      })
    ...
    getTableData (params) {
      ...
      net.apiGet(api.gatherDriver.list, params).then(data => {
        if (data && data.code === enums.NET_SUCCESS) {
    ...
    exportOrder () {
      ...
      str = api.gatherDriver.export + '?' + str
