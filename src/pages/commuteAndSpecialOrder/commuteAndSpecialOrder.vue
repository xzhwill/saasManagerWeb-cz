<template>
  <div class="common-container">
    ...
    <el-dialog title="行程退款" :visible.sync="dialogTableVisible" :append-to-body='true'>
...
<script>
import { customeBusOrderHeadSearch, customeBusOrderTableHead } from '@/utils/data'
...
export default {
  ...
  data () {
    return {
      formModules: customeBusOrderHeadSearch,
      tableList: [],
      page: {
        pageNum: 1,
        pageSize: 10,
  ...
  mounted () {
    ...
    this.$emit('getInfo', customeBusOrderHeadSearch)
    this.$emit('getTable', customeBusOrderTableHead)
  ...
  methods: {
    ...
    showDialog (val) {
      ...
      this.bussId = val.businessType
      this.getScheduleList(val.orderId, val.businessType)
    },
    /**
     * 获取订单退款列表数据
     * @param orderId：订单ID
     ...
     */
    getScheduleList (orderId, businessType) {
      net.apiGet(api.orderManage.orderTravelRefundList, { orderId, businessType }).then(data => {
    ...
    getTableData (params) {
      ...
      net.apiGet(api.orderManage.commuteOrderList, params).then(data => {
        if (data && data.code === enums.NET_SUCCESS) {
    ...
    exportOrder () {
      ...
      delete obj.pageNum
      delete obj.pageSize
      // net.apiExport(api.orderManage.commuteOrderOut, obj, '定制公交订单.xls')
      let str = ''
      for (let key in obj) {
      ...
      str = api.orderManage.commuteOrderOut + '?' + str
