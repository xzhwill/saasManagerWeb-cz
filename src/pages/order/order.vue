  ...
  <div class="common-container">
    <div class="page-nav-btn clearfix">
      <div class="page-nav-btn-item fl">
        <el-button type="primary" @click="exportOrder" :disabled="!+page.total" style="display: none;">导出</el-button>
      </div>
    </div>
    <!-- table -->
...
export default {
  ...
  data () {
    ...
    return {
      ...
      options: [],
      scheduleList: [],
      checkScheduleList: [], // 当设置勾选按钮时，放选中的列表
      showSchedulel: false,
      orderId: '', // 订单ID
      custId: '' // 用户ID
    }
  ...
  mounted () {
    ...
    this.rmList = setOperation('order')
  ...
  methods: {
     ...
     */
    getScheduleList (e) {
      net.apiGet(api.commuteOrder.travelList, { orderId: e }).then(data => {
    ...
    refundSchedule (e) {
      ...
      this.$confirm('此操作将取消选中行程, 是否继续?', '提示', {
        ...
        this.checkScheduleList.forEach(item => {
          if (item.travelId) travelIds.push(parseInt(item.travelId))
        })
    ...
    getTableData (params) {
      ...
      net.apiGet(api.commuteOrder.list, params).then(data => {
        if (data && data.code === enums.NET_SUCCESS) {
    ...
    exportOrder () {
      ...
      str = api.commuteOrder.exportOrder + '?' + str
